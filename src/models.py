...
class AttentionHead(nn.Module):
    ...
    def scaled_dot_product_attention(self, q, k, v):
        ...
        output = torch.matmul(weights.sum(dim=0), v)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int, num_attention_heads: int):
        ...
        self.output_linear = nn.Linear(
            in_features=num_attention_heads * d, out_features=d_model
        )
...
class FeedForward(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        first_layer = self.linear_1(x)
        second_layer = self.gelu(first_layer)
        x = self.linear_2(second_layer)
        return x
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        forward = self.layer_norm_2(x)
        ff = self.feed_forward(forward)
        x = x + ff
